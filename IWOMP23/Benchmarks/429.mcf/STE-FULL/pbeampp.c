...
NEXT:
    ...
    */
    ...
    for(arc_strip =arc2, itera=0  ; arc_strip < stop_arcs; arc_strip += nr_group*STRIP_SIZE, itera++)
        //for( ; arc < stop_arcs; arc += nr_group )
    {

        if (itera==(ceil(iterations*1.0/STRIP_SIZE)-1) && (iterations%STRIP_SIZE)!=0) size=iterations%STRIP_SIZE;
        else size=STRIP_SIZE;
        //printf("size %d\n",size);

        #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera)  shared(red_arr,cond_arr)  depend(out:cond_arr[itera*STRIP_SIZE:size])   //depend(inout:basket_size) // spec_private(basket_size) depend(sṕec_inout:basket_size)
        {
        ...
        #pragma omp task default(none) private(arc,red_cost,iter) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera) shared(basket_size,next_iter,cond_arr,basket_arr)  depend(in:cond_arr[itera*STRIP_SIZE:size]) depend(out:basket_arr[itera*STRIP_SIZE:size])  //depend(inout:basket_size) // spec_private(basket_size) depend(sṕec_inout:basket_size)
        {
...
Retry:
            ...
            for (arc=arc_strip,iter=0; ((arc-arc_strip)< STRIP_SIZE*nr_group) && (arc < stop_arcs); arc+=nr_group,iter++)
            {
                if(cond_arr[itera*STRIP_SIZE+iter] )
                {
                    ...
                    basket_sizeL++;
                    basket_arr[itera*STRIP_SIZE+iter]=basket_sizeL;
                }
            }
        ...
        #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter,basket_size) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera)  shared(red_arr,cond_arr,basket_arr,perm)  depend(in:basket_arr[itera*STRIP_SIZE:size])   //depend(inout:basket_size) // spec_private(basket_size) depend(sṕec_inout:basket_size)
	{
            for (arc=arc_strip,iter=0; ((arc-arc_strip)< STRIP_SIZE*nr_group) && (arc < stop_arcs); arc+=nr_group,iter++)
            {
                if(cond_arr[itera*STRIP_SIZE+iter])
                {
                    red_cost= red_arr[itera*STRIP_SIZE+iter];
                    basket_size= basket_arr[itera*STRIP_SIZE+iter];
