...
#ifdef _PROTO_
long primal_net_simplex( network_t *net , const long strip_size, const int nthreads)
#else
long primal_net_simplex(  net, strip_size, nthreads )
    network_t *net;
...
#endif
{
    ...
    while( !opt )
    {       
        if( (bea = primal_bea_mpp( m, arcs, stop_arcs, &red_cost_of_bea , strip_size, nthreads)) )
        {
            (*iterations)++;
