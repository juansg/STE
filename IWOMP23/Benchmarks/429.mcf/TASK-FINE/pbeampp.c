    ...
    */
        ...
        //for( ; arc < stop_arcs; arc += nr_group )
    {
	 ...
	 //if (itera==(ceil(iterations*1.0/STRIP_SIZE)-1) && (iterations%STRIP_SIZE)!=0) size=iterations%STRIP_SIZE; else size=STRIP_SIZE;
        #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter)  shared(basket_size,perm) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs)   depend(inout:basket_size) // spec_private(basket_size) depend(sṕec_inout:basket_size)
        {
