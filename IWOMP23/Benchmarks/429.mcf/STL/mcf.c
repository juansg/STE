...
#ifdef _PROTO_
long global_opt( const long strip_size, const int nthreads )
#else
long global_opt( strip_size, nthreads )
...
#endif
{
    ...
    while( new_arcs )
    {
        ...
        primal_net_simplex( &net, strip_size, nthreads );
...
#endif
{
    ...
    if ((parameters=fopen("_finfo_dataset","rt"))==NULL)
    {
    fprintf(stderr,"\nError: Can't find parameters!\n");
    return 1;
    }

    fscanf(parameters, "%ld %d", &strip_size, &nthreads);
    fclose(parameters);
    ...
    {
    //*/
    global_opt(strip_size,nthreads );
    ///*
    } 
    }
    //*/
