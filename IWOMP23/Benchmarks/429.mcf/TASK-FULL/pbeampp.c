    ...
    */
        ...
        //for( ; arc < stop_arcs; arc += nr_group )
    {
	 ...
	 if (itera==(ceil(iterations*1.0/STRIP_SIZE)-1) && (iterations%STRIP_SIZE)!=0) size=iterations%STRIP_SIZE; else size=STRIP_SIZE;
        #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter)  shared(red_arr,cond_arr) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera)    depend(out:cond_arr[itera*STRIP_SIZE:size]) // spec_private(basket_size) depend(sṕec_inout:basket_size)
        {
       ...
       #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter)  shared(basket_size,cond_arr,basket_arr) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera)  depend(inout:basket_size) depend(in:cond_arr[itera*STRIP_SIZE:size]) depend(out:basket_arr[itera*STRIP_SIZE:size]) // spec_private(basket_size) depend(sṕec_inout:basket_size)
	{
            for (arc=arc_strip,iter=0; ((arc-arc_strip)< STRIP_SIZE*nr_group) && (arc < stop_arcs); arc+=nr_group,iter++)
            {
                ...
                {
                    basket_size++;
		      basket_arr[itera*STRIP_SIZE+iter]=basket_size;                
		  }
            }
        }
 
        #pragma omp task default(none) private(arc,red_cost,cond1,cond2,iter,basket_size)  shared(perm,red_arr,cond_arr,basket_arr) firstprivate(arc_strip,STRIP_SIZE,nr_group,stop_arcs,itera) depend(in:basket_arr[itera*STRIP_SIZE:size])  // spec_private(basket_size) depend(sṕec_inout:basket_size)
        {
