...
// solver
void solver(float ***mat, int n, int m,float ***un){
  float diff = 0, temp;
  ...
  while (!done && (cnt_iter < MAX_ITER)){
    ...
    for (i = 1; i < n - 1; i++)
      for (j = 1; j < m - 1; j++){
	temp = (*mat)[i][j];
	(*un)[i][j] = 0.25 * ( (*mat)[i][j - 1] + (*mat)[i - 1][j] + (*mat)[i][j + 1] + (*mat)[i + 1][j]);
	diff = fmaxf(fabs((*un)[i][j] - temp),diff);
      }
    //printf("diff %f\n", diff);
