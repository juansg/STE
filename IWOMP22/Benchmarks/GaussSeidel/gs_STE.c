...
//printf("Task 1 inicio diff %f cnt %d next %d done %d \n", diff[cnt_iter],cnt_iter,next_iter,done);
                    for (int i = 1; i < n-BS; i+=BS)
                    {
                        for (int j = 1; j < m-BS; j+=BS)
                        {
                            ...
                            #pragma omp task  default(none) firstprivate(BS,m,n,i,j,cnt_iter,mat) shared( next_iter,done,diff)  depend(inout: (*mat)[i:BS][j:BS]) depend(in:(*mat)[i+BS:1][j:BS], (*mat)[i-1:1][j:BS] , (*mat)[i:BS][j-1:1], (*mat)[i:BS][j+BS:1])  depend(out:diff[cnt_iter])
                            {
...
*/
					     ...
					     //diff[cnt_iter]=fmaxf(fabs((*mat)[ii][jj] - temp),diff[cnt_iter]);		
                                        diffL = fmaxf(diffL,fabs((*mat)[ii][jj] - temp));
...
//printf("Task 1 fin diff %f cnt %d next %d done %d \n", diff[cnt_iter],cnt_iter,next_iter,done);
                ...
                #pragma omp task depend(in: diff[cnt_iter]) default(none) shared(diff,done,next_iter) firstprivate(cnt_iter)
                {
