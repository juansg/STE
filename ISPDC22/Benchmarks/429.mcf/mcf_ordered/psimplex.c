...
#ifdef _PROTO_
long primal_net_simplex( network_t *net , const int nthreads)
#else
long primal_net_simplex(  net, nthreads )
    network_t *net;
    const int nthreads;
#endif
{
    ...
    while( !opt )
    {       
        if( (bea = primal_bea_mpp( m, arcs, stop_arcs, &red_cost_of_bea , nthreads)) )
        {
            (*iterations)++;
