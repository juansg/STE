...
NEXT:
    ...
    #pragma omp parallel for schedule(static, 1) default(none) shared(next_iter_commit,basket_size) firstprivate(stop_arcs,nr_group,arc2,perm,STRIP_SIZE) private(red_cost,arc) num_threads(nthreads)
