...
void susan_smoothing(int three_by_three, uchar* in, float dt, 
                     int x_size, int y_size, uchar* bp, const long STRIP_SIZE, const int nthreads)
{
/* {{{ vars */
...
/* }}} */
  ...
  #pragma omp parallel for schedule(static,1) default(none) shared(next_iter_commit) firstprivate(dp,in,mask_size,x_size,bp,increment,y_size,out,STRIP_SIZE) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip,i) num_threads(nthreads) //reduction(+:n) //linear(seed:13)  //spec(n)

    for (is = mask_size; is < y_size-mask_size; is+=STRIP_SIZE){
        char execute_in_htm;
        unsigned status;
        
        uchar out_arr [STRIP_SIZE*(x_size-mask_size-mask_size)];
	...
	for (i=is; ((i-is)< STRIP_SIZE) && (i<(y_size-mask_size)); i++){
		...
		*/		
		...
		if (tmp==0)
			out_arr[count]=median(in,i,j,x_size);
		else
			out_arr[count]=((total-(centre*10000))/tmp);
	   ...
	   for (j=mask_size;j<x_size-mask_size;j++){		      
		count++;               
		out[(i-mask_size)*(x_size-2*mask_size)+j-mask_size]=out_arr[count];
...
/* }}} */
  ...
  switch (mode)
  {
    case 0:
      ...
      susan_smoothing(three_by_three,in,dt,x_size,y_size,bp,strip_size,nthreads);
