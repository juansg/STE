...
/* }}} */
  ...
  #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(dp,in,mask_size,x_size,bp,increment,out,y_size) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)  num_threads(nthreads) // linear(out:1) lastprivate(out)  
