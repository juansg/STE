...
/* }}} */
  ...
  {
  */
  #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit,out) firstprivate(dp,in,mask_size,x_size,bp,increment,y_size,STRIP_SIZE) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip,i)//,out_arr)  //reduction(+:n) //linear(seed:13)  //spec(n)

    for (is = mask_size; is < y_size-mask_size; is+=STRIP_SIZE){
        char execute_in_htm;
        unsigned status;
        
        uchar out_arr [STRIP_SIZE*(x_size-2*mask_size)];
