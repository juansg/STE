...
void susan_smoothing(int three_by_three, uchar* in, float dt, 
                     int x_size, int y_size, uchar* bp, const int nthreads)
{
/* {{{ vars */
...
/* }}} */
  ...
  #pragma omp parallel for ordered(1) default(none) schedule(auto) shared(out) firstprivate(dp,in,mask_size,x_size,bp,increment,i) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)  num_threads(nthreads) // linear(out:1) lastprivate(out)   
...
void main1(int argc, char* argv[], const int nthreads)
{
/* {{{ vars */
...
/* }}} */
  ...
  switch (mode)
  {
    case 0:
      ...
      susan_smoothing(three_by_three,in,dt,x_size,y_size,bp,nthreads);
