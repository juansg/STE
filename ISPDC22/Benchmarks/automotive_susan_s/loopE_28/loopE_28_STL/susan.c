    ...
    if (three_by_three==0)
    {
        ...
        */
        for (i=mask_size; i<y_size-mask_size; i++)
        {
            ...
            #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit,out) firstprivate(dp,in,mask_size,x_size,bp,increment,i,STRIP_SIZE) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip)//,out_arr)   //reduction(+:n) //linear(seed:13)  //spec(n) lastprivate(out)
