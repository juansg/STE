...
/* }}} */
  ...
  #pragma omp parallel for schedule(static,1) default(none) shared(next_iter_commit) firstprivate(dp,in,mask_size,x_size,bp,increment,i,out,STRIP_SIZE) private(area,total,dpt,j,tmp,brightness,x,y,cp,centre,ip,out_arr)  num_threads(nthreads) //reduction(+:n) //linear(seed:13)  //spec(n) lastprivate(out)
