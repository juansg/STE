...
void susan_edges(uchar* in, int* r, uchar* mid, uchar* bp,
                 int max_no, int x_size, int y_size, const long STRIP_SIZE, const int nthreads)
{
    float z;
    int   do_symmetry, i, j, m, n, a, b, x, y, w,  js;
    uchar c,*p,*cp;

    //int r_arr[STRIP_SIZE];
    ...
    for (i=3; i<y_size-3; i++)
    {
        ...
        #pragma omp parallel for schedule (static,1) default(none) shared(next_iter_commit) firstprivate(r,max_no,bp,x_size,in,i,STRIP_SIZE) private(n,cp,p,j) num_threads(nthreads)  //spec(n)

        for (js=3; js<x_size-3; js+=STRIP_SIZE)
        {
            char execute_in_htm;
            unsigned status;
...
Retry:
            ...
            for (j=js; j-js< STRIP_SIZE && j<x_size-3; j++)
            {
                cont++;
                n=100;
                p=in + (i-3)*x_size + j - 1;
            ...
            {
                cont++;
                if(cond_arr[cont]) r[i*x_size+j] =r_arr[cont];
            }
    ...
    case 1:
        ...
        else
        {
            ...
            if (three_by_three)
                ...
                susan_edges(in,r,mid,bp,max_no_edges,x_size,y_size,strip_size,nthreads);
            if(thin_post_proc)
                susan_thin(r,mid,x_size,y_size);
