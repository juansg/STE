...
{
    float z;
    int   do_symmetry, i, j, m, n, a, b, x, y, w,  js,cont;
    uchar c,*p,*cp;
        ...
        #pragma omp single
        {
            */
            for (i=3; i<y_size-3; i++)
            {
                ...
                #pragma omp taskloop grainsize(1) default(none) shared(next_iter_commit) firstprivate(r,max_no,bp,x_size,in,i,STRIP_SIZE) private(n,cp,p,j,cont)//,r_arr)   //spec(n)
