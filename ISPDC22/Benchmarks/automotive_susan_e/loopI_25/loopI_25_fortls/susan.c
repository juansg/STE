                 ...
                 int max_no, int x_size, int y_size, const long STRIP_SIZE, const int nthreads)
{
    ...
    #pragma omp parallel for schedule (static,1) default(none) shared(next_iter_commit) firstprivate(in,x_size,bp,max_no,r,y_size,STRIP_SIZE) private(j,p,cp,n,i,cont) num_threads(nthreads)  //spec(n)
