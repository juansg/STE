                 ...
                 int max_no, int x_size, int y_size, const long STRIP_SIZE, const int nthreads)
{
    ...
    //int r_arr[MAX_SS*594];
    //char cond_arr[MAX_SS*594];
    ...
    {
    */
    #pragma omp taskloop grainsize (1) default(none) shared(next_iter_commit,r) firstprivate(in,x_size,bp,max_no,y_size,STRIP_SIZE) private(j,p,cp,n,i,cont)//,r_arr)//,cond_arr)   //spec(n)
