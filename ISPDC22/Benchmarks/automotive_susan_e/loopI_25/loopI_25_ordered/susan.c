                 ...
                 int max_no, int x_size, int y_size, const int nthreads)
{
    ...
    #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(in,x_size,bp,max_no,y_size) shared(r) private(j,p,cp,n) num_threads(nthreads)  //spec(n)
