...
void susan_edges(uchar* in, int* r, uchar* mid, uchar* bp,
                 int max_no, int x_size, int y_size, const int nthreads)
{
    float z;
    ...
    #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(mid,r,max_no,bp,x_size,in,y_size) private(j,w,y,x,b,a,m,do_symmetry,z,n,c,cp,p) num_threads(nthreads)  //spec(n)
    ...
    case 1:
        ...
        else
        {
            ...
            if (three_by_three)
                ...
                susan_edges(in,r,mid,bp,max_no_edges,x_size,y_size,nthreads);
            if(thin_post_proc)
                susan_thin(r,mid,x_size,y_size);
