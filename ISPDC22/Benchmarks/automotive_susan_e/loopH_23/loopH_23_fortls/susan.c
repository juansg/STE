                 ...
                 int max_no, int x_size, int y_size, const long STRIP_SIZE, const int nthreads)
{
    ...
    #pragma omp parallel for schedule (static,1) default(none) num_threads(nthreads) shared(next_iter_commit) firstprivate(r,max_no,bp,x_size,in,y_size,STRIP_SIZE,mid) private(cont,i,j,w,y,x,b,a,m,do_symmetry,z,n,c,cp,p)//,mid_arr)   //spec(n)
