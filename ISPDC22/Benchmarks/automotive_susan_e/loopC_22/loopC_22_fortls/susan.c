...
uchar c,*p,*cp;
  ...
  for (i=4;i<y_size-4;i++){
    ...
    #pragma omp parallel for schedule (static,1) default(none) shared(next_iter_commit) firstprivate(mid,r,max_no,bp,x_size,in,i,STRIP_SIZE) private(cont,j,w,y,x,b,a,m,do_symmetry,z,n,c,cp,p) num_threads(nthreads)  //spec(n)
    
    for (js=4;js<x_size-4;js+=STRIP_SIZE)
    {
      char execute_in_htm;
      unsigned status;
      uchar mid_arr[STRIP_SIZE];        
      char cond_arr[MAX_SS]={0};
      ...
      for (j=js; j-js< STRIP_SIZE && j<x_size-4; j++){
                cont++;
