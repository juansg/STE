...
uchar c,*p,*cp;
  ...
  for (i=5;i<y_size-5;i++){
    ...
    #pragma omp parallel for schedule(static,1) firstprivate(max_no,bp,in,cgx,cgy,r,x_size,i,STRIP_SIZE) private(n,j,x,c,p,cp,divide,y,xx,yy,sq) default(none) shared(next_iter_commit) num_threads(nthreads)  //spec(n)
    for (js=5;js<x_size-5;js+=STRIP_SIZE) {
    	//iterations++;
       ...
       unsigned status;
		...
		for (j=js; ((j-js)< STRIP_SIZE) && (j<(x_size-5)); j++){
