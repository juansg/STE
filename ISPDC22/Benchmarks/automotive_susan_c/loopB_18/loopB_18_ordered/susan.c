...
void susan_corners(uchar* in, int* r, uchar* bp,
                   ...
                   int x_size, int y_size, const int nthreads)
{
int   n,x,y,sq,xx,yy,
      i,j,*cgx,*cgy;
float divide;
uchar c,*p,*cp;
  ...
  for (i=5;i<y_size-5;i++){
    ...
    #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(max_no,bp,in,cgx,cgy,r,x_size,i) private(n,x,c,p,cp,divide,y,xx,yy,sq)   num_threads(nthreads)  //spec(n)
    ...
    case 2:
      ...
      else
      {
        if(susan_quick)
          ...
          susan_corners(in,r,bp,max_no_corners,corner_list,x_size,y_size,nthreads);
