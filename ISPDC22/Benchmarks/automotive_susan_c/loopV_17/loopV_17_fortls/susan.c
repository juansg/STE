...
void susan_corners(uchar* in, int* r, uchar* bp,
                   ...
                   int x_size, int y_size, const long STRIP_SIZE, const int nthreads)
{
int   x,y,sq,xx,yy,
      i,j,*cgx,*cgy;
float divide;
uchar c,*p,*cp;
  ...
  #pragma omp parallel for schedule (static,1) default(none) shared(next_iter_commit,n) firstprivate(x_size,y_size,r,corner_list,cgx,cgy,in,STRIP_SIZE) private(j,x,i) num_threads(nthreads)  //spec(n)
  for (is=5;is<y_size-5;is+=STRIP_SIZE){
     
     
    //stick_this_thread_to_core(((i-5)/STRIP_SIZE)%NUM_THREADS);
    
    char execute_in_htm;
    unsigned status;
     
    //int * next_itr_commit=next_itr_commitg;
    //ALLOC(,int* ,ptr_n);
    //int nL;
    //int flag;
    //int j;
    ...
    case 2:
      ...
      else
      {
        if(susan_quick)
          ...
          susan_corners(in,r,bp,max_no_corners,corner_list,x_size,y_size,strip_size,nthreads);
