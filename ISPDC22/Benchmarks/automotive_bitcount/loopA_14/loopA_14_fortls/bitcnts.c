...
int main1(int argc, char *argv[], int print, const long STRIP_SIZE, const int nthreads)
{
    /*   clock_t start, stop; */
    /*  double ct = 0.0; */
    ...
    for (i = 0; i < FUNCS; i++)
    {
        ...
        */
        ...
        n=0;
        //seed=1;
        #pragma omp parallel for schedule(static,1) default(none) firstprivate(iterations,i,pBitCntFunc,STRIP_SIZE) private(seed,j,nL)  shared(next_iter_commit,n)  num_threads(nthreads) //reduction(+:n) //linear(seed:13)  //spec(n)
