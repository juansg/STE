...
int main1(int argc, char *argv[], int print, const int nthreads)
{
/*   clock_t start, stop; */
  /*  double ct = 0.0; */
...
*/
    ...
    n=0;
    //seed=1;
    #pragma omp parallel for ordered(1) default(none) schedule(auto) firstprivate(i,pBitCntFunc,iterations) private(seed,temp) shared(n)  num_threads(nthreads) //shared(iterations) //reduction(+:n) //linear(seed:13)  //spec(n)
