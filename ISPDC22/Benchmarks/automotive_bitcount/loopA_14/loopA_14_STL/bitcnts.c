...
int main1(int argc, char *argv[], int print, const long STRIP_SIZE, const int nthreads)
{
    ...
    for (i = 0; i < FUNCS; i++)
    {
        ...
        */
        ...
        #pragma omp taskloop grainsize(1)default(none) firstprivate(iterations,i,pBitCntFunc,STRIP_SIZE) private(seed,j,nL)  shared(next_iter_commit,n)  //reduction(+:n) //linear(seed:13)  //spec(n)
